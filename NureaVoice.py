...
class Jarvis:
    ...
    def respond(self, user_input):
        ...
        if "refresh news" in user_input_lower or "update news" in user_input_lower or "reload news" in user_input_lower or "get latest news" in user_input_lower or "refresh" in user_input_lower:
            self.show_todays_news()
            return
        ...
        pic_match = re.search(r'(?:display|show|view|get|see) (?:image|picture|photo|pic)(?: of)? (.+)', user_input, re.IGNORECASE)
        if pic_match:
        ...
        if "send" in user_input_lower and "message" in user_input_lower:
